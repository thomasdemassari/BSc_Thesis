# BSC Thesis: "The Effects of Monetary Policy on the Banking Industry"
# Owner: Thomas De Massari
# Last update: 2024-06-04

# I apologize in advance if some variables have names in Italian. The first version of this code
# was written in Italian, and with R-Studio I had trouble renaming some variables. I have added 
# comments to make everything clearer.
...
# SECTION 2:FUNCTIONS ----
# Set the p-value levels ----
# This function sets a sign based on the significance of the estimated value
...
# Exogeneity (simple correlation between residuals and regressors) ----
# It takes as input the residuals from the OLS model and a dataset (cbind) with all the regressors. 
# If at least one of these is correlated with the error term, it returns FALSE; otherwise, it 
# returns TRUE
...
# OLS model estimates -----
# The function takes individual variables as input in a panel. Therefore, a filter must be provided.
# It is important to follow the syntax below:
# 1. Dependent variable
# 2. Regressors passed as cbind
# 3. Dummies passed as cbind. Can be omitted
# 4. Names_of_models: names of the models to be estimated. Must be included even if only one model is specified
# 5. Filter: variable to filter the panel
# 6. intercept = TRUE if you want to include the intercept
# 7. differences = TRUE if you want to use first differences for the model (both for regressors
...
OLSregression = function(dipendente, regressori, dummy, names_of_models, filtro, intercetta = TRUE, rendimenti = FALSE){
  ...
  number_of_correlations = choose(number_of_coefficients, 2) # This will be needed later for the multicollinearity matrix
  ...
  # Coefficient matrix (with corresponding p-values)
  ...
  # Test matrix
  # NB: Only the tests I have chosen to perform will be calculated. The user cannot modify
  # this option
  test_names = c("R2", "R2 adj", "Test F (pvalue)", "Exogeneity","Goldfeld-Quandt (pvalue)", "White (pvalue)", 
                 "Shapiro-Wilk (pvalue)", "Jarque-Bera (pvalue)", "Kurtosis", "Skewness", "Durbin-Watson (pvalue)", 
  ...
  # Matrix where I will store the correlations between regressors
  ...
  # Adjust the matrices (I need to remove an observation)
  if (rendimenti == TRUE){
    res_matrix = res_matrix[, -ncol(res_matrix)]
  }
  
  formula = NA # Model formula
  models = NA  # Model object
  ...
  for (i in 1:length(names_of_models)){
    if(rendimenti == TRUE){
      ...
      # Check for negative values (since I need to take logarithms, these cannot be present)
      # In the case of this thesis, only interest rates might be negative, but the code
      # is written to generalize and perform the check on all variables.
      # Given the nature of the problem, if a value is negative, it is replaced with 0.00001 (a small value,
      # very close to 0, chosen arbitrarily)
      ...
      if (!missing(dummy)){
        # Dummy and interaction between dummies and explanatory variables
      ...
      if (!missing(dummy)){
        # Dummy and interactions between dummies and explanatory variables
        for (j in 1:length(dummy[1, ])){
          d_name = paste0("d", j, "_model")
          assign(d_name, dummy[, j][filtro == i])
          dataframe_tmp[[d_name]] = get(d_name)
    ...
    # Set of regressors: x, d, xd
    # (needed later for the endogeneity check as simple correlation)
    ...
    # Save the model formula and the model
    formula = c(formula, model_OLS$call)
    models = c(models, model_OLS)
    
    # Correction of standard errors with HC3
    ...
    # Exogeneity (calculated as simple correlation between regressors and residuals)
    ...
    # NORMALITY TEST
    # Shapiro-Wilk, Jarque-Bera, Curtis, Skewness
    ...
    # TEST FOR ABSENCE OF STRUCTURAL BREAKS
...
# Function to obtain model estimates and save them in a matrix ----
# This function is useful for generalizing the subsequent code to avoid repeating the same instructions.
# It definitely works for SUR.
# Returns the matrix that is passed in at the beginning. It is important that the matrix has the names
# of the estimated models on the rows and the parameters on the columns.
...
# SUR OR NOT SUR -----
# The function takes as input the correlation matrix (complete, including diagonal and
# duplications) and returns the test statistic and p-value to test
# H0: cov12 = cov13 = ... = cov_nm. If rejected, it means that the covariances (and hence correlations)
# are different from 0, and thus SUR should be used.
...
# Function to apply necessary tests for SUR ----
# It is important that the variables are passed using cbind and that the dependent variable is also included.
# Unlike the other function, this one works only for the specific case at hand.
...
test_SUR  = function(modello, dipendente, regressori, filtro, names_of_models, number_of_coefficients, rendimenti = FALSE){
  ...
  for(j in 1:nrow(test_matrix)){
    # Select the model among the SUR models that was passed as input
    model = modello$eq[[j]]
    ...
    # Set of regressors
    # (needed later for the exogeneity check as simple correlation)
    ...
    # Cannot calculate the F test
    test_matrix[j, 3] = "--NA--"
    
    # Exogeneity (calculated as simple correlation between regressors and residuals)
    exogeneity_justcorr = exogeneity(residuals(model), regressori_model)
    test_matrix[j, 4] = as.character(paste(exogeneity_justcorr, collapse = " ")) 
    # p-value of the Goldfeld-Quandt test (to test the hypothesis of absence of heteroscedasticity)
    gq_pvalue = as.numeric(gqtest(model)[5])
    test_matrix[j, 5] = paste(round(gq_pvalue, 5), sign_pvalue(gq_pvalue))
    # p-value of the White test (to test the hypothesis of absence of heteroscedasticity)
    white_pvalue = as.numeric(white_test(model)[2])
    test_matrix[j, 6] = paste(round(white_pvalue,5), sign_pvalue(white_pvalue))
    # p-value of the Shapiro-Wilk test (to test the hypothesis of normality)
    ...
    # p-value of the Jarque-Bera test (to test the hypothesis of normality)
    ...
    # p-value of the Durbin-Watson test (to test the hypothesis of absence of autocorrelation among residuals)
    ...
    # p-value of the CUSUM test (to test for the absence of structural breaks)
    ...
    # p-value of the Ramsey RESET test (to check the functional form of the model)
...
# Section 3a: PANELS ----
# Balance sheet data and interest rates (personal elaboration, see Python code)
data_fs = read.csv("financial_statement_panel.csv") 
# Stock price data, market indices, and interest rates (source: personal elaboration, see Python code)
data_sp = read.csv("stock_price_panel.csv")

# Section 3b: EUROZONE and ECB ----
# European inflation data (source: ECB)
...
# ECB balance sheet, annual (source: ECB)
...
# Monthly Euribor with complete historical series (source: ECB)
...
# Annual Euribor with complete historical series (source: my elaboration on ECB data)
...
# Deposit rate, monthly (source: FRED)
...
# Overnight rate, monthly (source: FRED)
...
# Correlation between inflation and euribor
...
# SECTION 4a: STATIONARITY TEST  ----
# Stationarity test on annual interest rate ----
# Matrix where I will save the test results
...
# Libor
libor = data_fs$InterestRate[data_fs$Bank == "hsbc"]                      
plot(ts(libor, start = 2006)) 
intrate_stationarity[1, 1] = as.numeric(kpss.test(libor, "Trend")[3])     # pvalue of kpss 
intrate_stationarity[1, 2] = as.numeric(pp.test(libor)[4])                # pvalue of pp
intrate_stationarity[1, 3] = as.numeric(adf.test(libor)[4])               # pvalue of adf
intrate_stationarity[1, 4] = as.numeric(arima(libor, c(1,0,0))$coef[1])   # coef of AR(1)
acf(libor, main = "ACF Libor")
# Euribor
euribor = data_fs$InterestRate[data_fs$Bank == "ucg"]                     # Euribor
plot(ts(euribor, start = 2006))
intrate_stationarity[2, 1] = as.numeric(kpss.test(euribor, "Trend")[3])   # pvalue of kpss
intrate_stationarity[2, 2] = as.numeric(pp.test(euribor)[4])              # pvalue of pp
intrate_stationarity[2, 3] = as.numeric(adf.test(euribor)[4])             # pvalue of adf
intrate_stationarity[2, 4] = as.numeric(arima(euribor, c(1,0,0))$coef[1]) # coef of AR(1)
acf(euribor, main = "ACF Euribor")

# Result
print(intrate_stationarity)
nome_file = "Interest rate (annual).csv"
...
# Stationarity test on ROA, NII, OI, PROV ----
# ID of banks
index = c(1:14)  
...
for (i in 1:length(index)){
  ...
  roa_stationarity[i, 1] = as.numeric(kpss.test(roa)[3])                           # pvalue of kpss
  roa_stationarity[i, 2] = as.numeric(pp.test(roa)[4])                             # pvalue of pp
  roa_stationarity[i, 3] = as.numeric(adf.test(roa)[4])                            # pvalue of adf
  roa_stationarity[i, 4] = as.numeric(as.numeric(arima(roa, c(1,0,0))$coef[1]))    # coef of AR(1)
  
  # NII
  nii = data_fs$NII_Assets[data_fs$ID == i]
  nii_stationarity[i, 1] = as.numeric(kpss.test(nii)[3])                           # pvalue of kpss
  nii_stationarity[i, 2] = as.numeric(pp.test(nii)[4])                             # pvalue of pp
  nii_stationarity[i, 3] = as.numeric(adf.test(nii)[4])                            # pvalue of adf
  nii_stationarity[i, 4] = as.numeric(as.numeric(arima(nii, c(1,0,0))$coef[1]))    # coef of AR(1)
  
  # OI
  oi = data_fs$OI_Assets[data_fs$ID == i]
  oi_stationarity[i, 1] = as.numeric(kpss.test(oi)[3])                             # pvalue of kpss
  oi_stationarity[i, 2] = as.numeric(pp.test(oi)[4])                               # pvalue of pp
  oi_stationarity[i, 3] = as.numeric(adf.test(oi)[4])                              # pvalue of adf
  oi_stationarity[i, 4] = as.numeric(as.numeric(arima(oi, c(1,0,0))$coef[1]))      # coef of AR(1)
  
  # PROV
  prov = data_fs$PROV_Assets[data_fs$ID == i]
  prov_stationarity[i, 1] = as.numeric(kpss.test(prov)[3])                          # pvalue of kpss
  prov_stationarity[i, 2] = as.numeric(pp.test(prov)[4])                            # pvalue of pp
  prov_stationarity[i, 3] = as.numeric(adf.test(prov)[4])                           # pvalue of adf
  prov_stationarity[i, 4] = as.numeric(as.numeric(arima(prov, c(1,0,0))$coef[1]))   # coef of AR(1)
}
...
# Stationarity test on central bank reserves ----
# Matrix where I will save the test results
...
for (i in 1:length(index)){
  ...
  dcb_stationarity[i, 1] = as.numeric(kpss.test(dcb, "Trend")[3])                  # pvalue of kpss
  dcb_stationarity[i, 2] = as.numeric(pp.test(dcb)[4])                             # pvalue of pp
  dcb_stationarity[i, 3] = as.numeric(adf.test(dcb)[4])                            # pvalue of adf
  dcb_stationarity[i, 4] = as.numeric(as.numeric(arima(dcb, c(1,0,0))$coef[1]))    # coef of AR(1)
  
  # plot(ts(dcb, start = c(2006,1), frequency = 1), main = paste("DCB - ", banks_acronym[i]))
  acf(dcb, main = paste("ACF - Reserve of", banks_acronym[i]))
}
par(mfrow = c(1,1))

# Result
print(dcb_stationarity)
nome_file = "central bank reserves.csv"
...
# Stationarity test on stock prices ----
...
for (i in 1:length(index)){
    ...
    pricestock_stationarity[i, 1] = as.numeric(kpss.test(sp)[3])                           # pvalue of kpss
    pricestock_stationarity[i, 2] = as.numeric(pp.test(sp)[4])                             # pvalue of pp
    pricestock_stationarity[i, 3] = as.numeric(adf.test(sp)[4])                            # pvalue of adf
    pricestock_stationarity[i, 4] = as.numeric(as.numeric(arima(sp, c(1,0,0))$coef[1]))    # coef of AR(1)
...
# Stationarity test on interest rates (monthly) ----
# Matrix where I will save the test results
...
# Libor
libor_monthly = data_sp$InterestRate[data_sp$Bank == "hsbc"]                              # Libor
plot(ts(libor_monthly, start = c(2006, 1), frequency = 12)) 
intrate_monthly_stationarity[1, 1] = as.numeric(kpss.test(libor_monthly, "Trend")[3])     # pvalue of kpss
intrate_monthly_stationarity[1, 2] = as.numeric(pp.test(libor_monthly)[4])                # pvalue of pp
intrate_monthly_stationarity[1, 3] = as.numeric(adf.test(libor_monthly)[4])               # pvalue of adf
intrate_monthly_stationarity[1, 4] = as.numeric(arima(libor_monthly, c(1,0,0))$coef[1])   # coef of AR(1)
acf(libor_monthly)
# Euribor
euribor_monthly = data_sp$InterestRate[data_sp$Bank == "ucg"]                             # Euribor
plot(ts(euribor_monthly, start = c(2006, 1), frequency = 12)) 
intrate_monthly_stationarity[2, 1] = as.numeric(kpss.test(euribor_monthly, "Trend")[3])   # pvalue of kpss
intrate_monthly_stationarity[2, 2] = as.numeric(pp.test(euribor_monthly)[4])              # pvalue of pp
intrate_monthly_stationarity[2, 3] = as.numeric(adf.test(euribor_monthly)[4])             # pvalue of adf
intrate_monthly_stationarity[2, 4] = as.numeric(arima(euribor_monthly, c(1,1,0))$coef[1]) # coef of AR(1)
acf(euribor_monthly)

# Result
print(intrate_monthly_stationarity)    
nome_file = "Interest rate (monthly).csv"
write.csv(intrate_stationarity, paste0("/Users/thomasdemassari/Library/CloudStorage/OneDrive-Personal/Università/BSc/Tesi/Data/Results/Test di stazionarietà/",nome_file))

# Stationarity test on the market index ----
...
for (i in 1:6){
  ...
  marketindex_stationarity[i, 1] = as.numeric(kpss.test(m_index)[3])                           # pvalue of kpss
  marketindex_stationarity[i, 2] = as.numeric(pp.test(m_index)[4])                             # pvalue of pp
  marketindex_stationarity[i, 3] = as.numeric(adf.test(m_index)[4])                            # pvalue of adf
  marketindex_stationarity[i, 4] = as.numeric(as.numeric(arima(m_index, c(1,0,0))$coef[1]))    # coef of AR(1)
...
# Stationarity test on the differences of log stock prices ----
...
for (i in 1:length(index)){
  ...
  logdiff_pricestock_stationarity[i, 1] = as.numeric(kpss.test(diffs_log_sp)[3])                           # pvalue of kpss
  logdiff_pricestock_stationarity[i, 2] = as.numeric(pp.test(diffs_log_sp)[4])                             # pvalue of pp
  logdiff_pricestock_stationarity[i, 3] = as.numeric(adf.test(diffs_log_sp)[4])                            # pvalue of adf
  logdiff_pricestock_stationarity[i, 4] = as.numeric(as.numeric(arima(diffs_log_sp, c(1,0,0))$coef[1]))    # coef of AR(1)
...
# Stationarity test on the differences of log market indices ----
...
for (i in 1:6){
  ...
  logdiff_marketindex_stationarity[i, 1] = as.numeric(kpss.test(logdiff_m_index)[3])                           # pvalue of kpss
  logdiff_marketindex_stationarity[i, 2] = as.numeric(pp.test(logdiff_m_index)[4])                             # pvalue of pp
  logdiff_marketindex_stationarity[i, 3] = as.numeric(adf.test(logdiff_m_index)[4])                            # pvalue of adf
  logdiff_marketindex_stationarity[i, 4] = as.numeric(as.numeric(arima(logdiff_m_index, c(1,0,0))$coef[1]))    # coef of AR(1)
...
nome_file = "MarketIndex - difflog.csv"
...
# Stationarity test on the differences of log monthly interest rates ----
...
# Libor 
log_libor_monthly = log(data_sp$InterestRate[data_sp$Bank == "hsbc"])                                       # libor
logdiffs_libor_monthly = diff(log_libor_monthly)
plot(ts(logdiffs_libor_monthly, start = c(2006, 1), frequency = 12)) 
logdiffs_intrate_monthly_stationarity[1, 1] = as.numeric(kpss.test(logdiffs_libor_monthly, "Trend")[3])     # pvalue of kpss
logdiffs_intrate_monthly_stationarity[1, 2] = as.numeric(pp.test(logdiffs_libor_monthly)[4])                # pvalue of pp
logdiffs_intrate_monthly_stationarity[1, 3] = as.numeric(adf.test(logdiffs_libor_monthly)[4])               # pvalue of adf
logdiffs_intrate_monthly_stationarity[1, 4] = as.numeric(arima(logdiffs_libor_monthly, c(1,0,0))$coef[1])   # coef of AR(1)
acf(logdiffs_libor_monthly)

# Euribor
# Since the Euribor is negative in some observations (minimum value -0.58%, very close to 0),
# and since I cannot compute the log of a negative number, I set it to 0 where Euribor is negative.
# Note: I set it to a value close to 0 to avoid log(0) = -inf
...
logdiffs_intrate_monthly_stationarity[2, 1] = as.numeric(kpss.test(logdiffs_euribor_monthly, "Trend")[3])   # pvalue of kpss
logdiffs_intrate_monthly_stationarity[2, 2] = as.numeric(pp.test(logdiffs_euribor_monthly)[4])              # pvalue of pp
logdiffs_intrate_monthly_stationarity[2, 3] = as.numeric(adf.test(logdiffs_euribor_monthly)[4])             # pvalue of adf
logdiffs_intrate_monthly_stationarity[2, 4] = as.numeric(arima(logdiffs_euribor_monthly, c(1,1,0))$coef[1]) # coef of AR(1)
acf(logdiffs_euribor_monthly)

# Result
print(logdiffs_intrate_monthly_stationarity)
nome_file = "Interest rate (monthly) - difflog.csv"
...
# SECTION 4b: COINTEGRATION TEST ----
# Cointegration test on stock prices ----
...
# Critical value (MacKinnon from "Introduction to Modern Time Series Analysis")
cv3 = -3.74
...
par(mfrow = c(1,1))
nome_file = "Model with stock ~ intrate + mkt.csv"
...
# Models with central bank reserves
...
# Models without central bank reserves
...
# Save the coefficients and tests in a CSV file
...
# Error analysis and check if it is appropriate to use SUR ----
# Matrices where I will save the results
...
# SUR ----
# Preparation of datasets for SUR
...
for (i in 1:14){
  ...
  # Equations with central bank reserves
  ...
  # Equations without central bank reserves
...
# Save the results in CSV for easier data access
...
plot(ts(roa_median, start = 2006), ylim = c(min(roa_q05per), max(roa_q95per)), 
     main = "ROA Trend", ylab = "ROA (%)", xlab = "Year")
...
axis(1, at = c(2006:2023), las = 1)
# Legend
legend("bottomright",  legend = c("Median", "25th-75th Percentile Area", "5th-95th Percentile Area"),  
...
plot(ts(nii_median, start = 2006), ylim = c(min(nii_q05per), max(nii_q95per)), 
     main = "NII Trend", ylab = "NII(t)/Assets(t-1) (%)", xlab = "Year")
...
axis(1, at = c(2006:2023), las = 1)
# Legend
legend("topright",  legend = c("Median", "25th-75th Percentile Area", "5th-95th Percentile Area"),  
       col = c("black", "gray80", "gray90"),  lty = c(1, NA, NA), lwd = c(2, NA, NA),
       fill = c(NA, "gray80", "gray90"), border = NA, cex = 0.8, bty = "n")

# OI Trend
plot(ts(oi_median, start = 2006), ylim = c(min(oi_q05per), max(oi_q95per)), 
     main = "OI Trend", ylab = "OI(t)/Assets(t-1) (%)", xlab = "Year")
...
plot(ts(prov_median, start = 2006), ylim = c(min(prov_q05per), max(prov_q95per)), 
     main = "PROV Trend", ylab = "PROV(t)/Assets(t-1) (%)", xlab = "Year")
...
for (i in 1:5){
  ...
  
  {
    ...
    if (names_list[i] != "STOCKS"){
      ...
      barplot(matrix_b2, beside = TRUE, col = c("black", "grey"), legend.text = c("s.e. OLS", "s.e. HC3"),
              args.legend = list(x = "topleft", bty = "n"), 
              main = paste("Standard error of b_mkt in the model with", names_list[i]))
    }
  }
  dev.off()
  par(mfrow = c(1,1))
...
# ECB - Interest Rate Corridor ----
plot(cbind(ecb_depositrate_m_9923, ecb_overnightrate_m_9923, euribor_m_9923, ecb_mro_m_9923), plot.type = "single",
     xlab = "Years", ylab = "Interest Rate (%)", col = c("black", "grey", "black", "grey"), lty = c("solid", "solid", "dashed", "dotdash"), lwd = 2)
abline(h = 0, col = "black", lty = "dotted")
legend("bottomleft", legend = c("Deposit Rate", "Marginal Refinancing Rate", "Euribor", "Main Refinancing Rate"), 
       col = c("black", "grey", "black", "grey"), lty = c("solid", "solid", "dashed", "dotdash"), lwd = 2)
axis(1, at = c(1999:2023), las = 1)

# Analysis of ECB Reserves and Euribor Trend ----
plot(ecb_deposit, plot.type = "single", ylab = "Deposits at ECB (Millions of euros)", xlab = "Years", lwd = 2, xaxt = "n")
par(new = TRUE)
...
legend("top", legend = c("Deposits at ECB", "Euribor"), col = c("black", "grey"), lty = 1, xjust = 0.5, yjust = 1.5, bty = "n")
abline(h = 0, lty = "dashed")

# Example with Homoscedasticity and Heteroscedasticity -----
x = runif(50, 2, 10)
...
# Performance of HSBC and Lloyds Returns ----
...
par(mfrow = c(1,2))
hist(return_hsbc, main = "Distribution of HSBC Returns", xlab = "Returns (%)", ylab = "Frequency")
text(x = min(return_hsbc)+4, y = max(hist(return_hsbc, plot = FALSE)$counts)-2, 
     labels = paste0("Median = ", round(median(return_hsbc), 3), "%"), pos = 3, cex = 1, col = "black")
text(x = min(return_hsbc)+4, y = max(hist(return_hsbc, plot = FALSE)$counts)-4, 
     labels = paste0("Mean = ", round(mean(return_hsbc), 3), "%"), pos = 3, cex = 1, col = "black")
...
hist(return_lloy, main = "Distribution of Lloyds Returns", xlab = "Returns (%)", ylab = "Frequency")
text(x = min(return_lloy)+4, y = max(hist(return_lloy, plot = FALSE)$counts)-2, 
     labels = paste0("Median = ", round(median(return_lloy), 3), "%"), pos = 3, cex = 1, col = "black")
text(x = min(return_lloy)+4, y = max(hist(return_lloy, plot = FALSE)$counts)-5, 
     labels = paste0("Mean = ", round(mean(return_lloy), 3), "%"), pos = 3, cex = 1, col = "black")
...
for (i in 1:14){
  ...
  # res = lm_prov$res[i, ]
  # res = lm_azioni$res[i, ]
  qqnorm(res, main = paste("QQ Plot - ROA of", banks_acronym[i]), xlab = "Theoretical Quantiles", ylab = "Observed Quantiles")
...
# Simulation:
{
  last_time = Sys.time()  # Time at the start of the cycle
  time_elapsed = 0
  intervallo = 15 # Seconds that pass between each print to track progress in the cycle
  
  set.seed(226091)
  obs = 18       # Number of observations
  reps = 1000000 # Number of simulation
  
  # Matrices where I will save the p-values of the tests
  ...
  for (i in 1:reps){
    ...
    # Simulations of a non stationary process
    rw = 5 + cumsum(rnorm(obs, mean = 0, sd = 1))
    
    # test
    results_02[i, 1] = as.numeric(kpss.test(ar_02)[3])                             # pvalue of kpss
    results_02[i, 2] = as.numeric(pp.test(ar_02)[4])                               # pvalue of pp
    results_02[i, 3] = as.numeric(adf.test(ar_02)[4])                              # pvalue of adf
    
    # Checking the p-values
    # If it is less than 0.05, add 1 to the rejection matrix; otherwise, leave 0
    j = 0
    for (j in 1:3){
    ...
    # test
    results_05[i, 1] = as.numeric(kpss.test(ar_05)[3])                             # pvalue of kpss
    results_05[i, 2] = as.numeric(pp.test(ar_05)[4])                               # pvalue of pp
    results_05[i, 3] = as.numeric(adf.test(ar_05)[4])                              # pvalue of adf
    ...
    # test
    results_08[i, 1] = as.numeric(kpss.test(ar_08)[3])                             # pvalue of kpss
    results_08[i, 2] = as.numeric(pp.test(ar_08)[4])                               # pvalue of pp
    results_08[i, 3] = as.numeric(adf.test(ar_08)[4])                              # pvalue of adf
    ...
    # test
    results_095[i, 1] = as.numeric(kpss.test(ar_095)[3])                           # pvalue of kpss
    results_095[i, 2] = as.numeric(pp.test(ar_095)[4])                             # pvalue of pp
    results_095[i, 3] = as.numeric(adf.test(ar_095)[4])                            # pvalue of adf
    ...
    # test
    results_1[i, 1] = as.numeric(kpss.test(rw)[3])                                 # pvalue of kpss
    results_1[i, 2] = as.numeric(pp.test(rw)[4])                                   # pvalue of pp
    results_1[i, 3] = as.numeric(adf.test(rw)[4])                                  # pvalue of adf
    ...
    # Adding instructions to track progress in the cycle, as it will likely take a while to simulate 
    # large samples
    # Time elapsed
    current_time = Sys.time()  
    time_elapsed = as.numeric(difftime(current_time, last_time, units = "secs")) 
    
    # If more than 10 seconds have passed since the last print, print the progress
    if (time_elapsed >= intervallo) {
      cat("Elapsed time:", round(as.numeric(difftime(current_time, last_time, units = "secs")), 1), "seconds and currently at simulation number:", i, "\n")
  ...
  par(mfrow = c(2,2))
  # KPSS
  plot(ts(powers[, "kpss"], start = 1), type = "l", xaxt = "n", xlab = "", main = "Rejection of KPSS", 
  ...
  # PP
  plot(ts(powers[, "pp"], start = 1), type = "l", xaxt = "n", xlab = "", main = "Rejection of PP",
       ylab = "Rejection percentage")
  axis(1, at = c(1:5), labels = c("phi = 0.2", "phi = 0.5", "phi = 0.8", "phi = 0.95", "phi = 1"), las = 2)
  points(1:5, powers[, "pp"], pch = 1, col = "black", cex = 1)
  # ADF
  plot(ts(powers[, "adf"], start = 1), type = "l", xaxt = "n", xlab = "", main = "Rejection of ADF",
       ylab = "Rejection percentage")
  axis(1, at = c(1:5), labels = c("phi = 0.2", "phi = 0.5", "phi = 0.8", "phi = 0.95", "phi = 1"), las = 2)
...
# Homoscedasticity test in small samples ------
{
  last_time = Sys.time()  
  time_elapsed = 0
  intervallo = 15 
  ...
  # NB: by difference I mean (s.e. White - s.e. OLS) / (s.e. OLS), so how much larger/smaller
  # the White standard errors are compared to those estimated with OLS
  
  for (i in 1:rep){
    # Simulation of the two processes
    x = runif(obs, 2, 10)
...
# Normality test in small samples ----
{
  obs = 18
  N = 10^6
  ...
  for (i in 1:N){
    ...
    # Normality test for a normal process
    ...
    # Normality test for a non normal process
...
# Distribution of B (SUR) with non-normal errors and with n = 216 -----
{
  reps = 10^6
  obs = 216
  ...
  # Non-normal and correlated errors (using SUR)
  for (i in 1:reps){
